      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
          {items.map((item, i) => (
            <div
              key={item.title}
              className="glass-card-elevated flex items-start gap-4 rounded-2xl p-6"
              style={{ animationDelay: `${i * 100}ms` }}
            >
              <div
                className={`flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl ${item.bgColor} ring-1 ring-white/50 dark:ring-white/10`}
              >
