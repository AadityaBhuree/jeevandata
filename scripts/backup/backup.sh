...
REDIS_HOST="${REDIS_HOST:-redis}"; REDIS_PORT="${REDIS_PORT:-6379}"; REDIS_PASSWORD="${REDIS_PASSWORD:-redis_secret}"
...
if PGPASSWORD="$PG_PASSWORD" pg_dump -h "$PG_HOST" -p "$PG_PORT" -U "$PG_USER" -d "$PG_DB" \
    -Fc -f "$WORK/postgres.dump" 2>/dev/null; then
...
if [ "$PG_BASEBACKUP" = "true" ]; then
  ...
  if PGPASSWORD="$PG_PASSWORD" pg_basebackup -h "$PG_HOST" -p "$PG_PORT" -U "$PG_USER" \
      -D "$WORK/pg_base" -Fp -Xs 2>/dev/null; then
...
if redis-cli -h "$REDIS_HOST" -p "$REDIS_PORT" -a "$REDIS_PASSWORD" --no-auth-warning \
    --rdb "$WORK/redis.rdb" >/dev/null 2>&1; then
...
for col in $QDRANT_COLLECTIONS; do
  ...
  for _ in $(seq 1 12); do
    ...
    snap_name="$(printf '%s' "$snap_json" | sed -n 's/.*"result":{[^}]*"name":"\([^"]*\)".*//p')"
    [ -n "$snap_name" ] && break
    sleep 5
  done
  if [ -z "$snap_name" ]; then
...
if [ -d "$PG_WAL_DIR" ] && [ -n "$(ls -A "$PG_WAL_DIR" 2>/dev/null)" ]; then
  mkdir -p "$WORK/wal"
  log "PostgreSQL: copying WAL archive to staging..."
  if cp -rn "$PG_WAL_DIR"/. "$WORK/wal/"; then
...
if mc alias set "$MC_ALIAS" "$S3_ENDPOINT" "$S3_ACCESS_KEY" "$S3_SECRET_KEY" >/dev/null 2>&1 \
   && mc mb "$MC_ALIAS/$S3_BUCKET" --ignore-existing >/dev/null 2>&1 \
   && mc cp -r "$WORK" "$MC_ALIAS/$S3_BUCKET/$STAMP/" >/dev/null 2>&1; then
  log "Upload OK"
else
