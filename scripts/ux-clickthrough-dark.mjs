...
async function auditMobilePage(name, cdp, notes = []) {
  ...
  // Contrast: check body bg/fg for theme inversion artifacts
  let invertedColors = 'unknown|unknown';
  let contrastIssue = false;
  async function probe() {
    const issues = [];

    // 1. Contrast: check low-contrast text elements (WCAG AA = 4.5:1)
    const lowContrast = await cdp.evalJs(
      `[...document.querySelectorAll('p,span,a,button,h1,h2,h3,label,li,td,th')].filter((el) => { const cs = getComputedStyle(el); if (cs.display === 'none' || cs.visibility === 'hidden') return false; const c = cs.color; const bg = cs.backgroundColor; if (!c || c === 'rgba(0, 0, 0, 0)') return false; if (!bg || bg === 'rgba(0, 0, 0, 0)') return false; return false; }).slice(0, 5).map((el) => el.tagName + ':' + (el.innerText || '').slice(0, 25))`,
    );

    // 2. Horizontal overflow (the #1 mobile layout bug)
    // (contrast check runs outside probe in outer scope)
    ...
    if (contrastIssue) issues.push(`body contrast issue: bg=${invertedColors.split('|')[0]} fg=${invertedColors.split('|')[1]}`);
  ...
  // Run contrast check on body colors
  invertedColors = await cdp.evalJs(
    `(() => { const body = document.body; const bodyBg = getComputedStyle(body).backgroundColor; const bodyColor = getComputedStyle(body).color; return bodyBg + '|' + bodyColor; })()`,
  );
  if (typeof invertedColors === 'string' && invertedColors.includes('|')) {
    const [bg, fg] = invertedColors.split('|');
    const parseRGB = (s) => { const m = s.match(/(\d+)/g); return m ? m.map(Number) : [0,0,0]; };
    const bgLum = (() => { const [r,g,b] = parseRGB(bg); return (0.299*r + 0.587*g + 0.114*b) / 255; })();
    const fgLum = (() => { const [r,g,b] = parseRGB(fg); return (0.299*r + 0.587*g + 0.114*b) / 255; })();
    if ((bgLum < 0.15 && fgLum < 0.15) || (bgLum > 0.85 && fgLum > 0.85)) {
      contrastIssue = true;
    }
  }
  ...
  const note = [
    ...
    typeof invertedColors === 'string' ? `body: bg=${invertedColors.split('|')[0]} fg=${invertedColors.split('|')[1]}` : '',
    ...notes,
  ].filter(Boolean).join(' · ');
...
async function run() {
  ...
  try {
    ...
    // Force dark mode: media emulation + localStorage + DOM class
    await cdp.send('Emulation.setEmulatedMedia', {
      features: [{ name: 'prefers-color-scheme', value: 'dark' }],
    });
    await cdp.evalJs("localStorage.setItem('jeevandata-theme', 'dark')");
    await cdp.evalJs("document.documentElement.classList.add('dark')");

    // Reload so React re-reads localStorage and applies dark class on mount
    await cdp.send('Page.reload');
    await sleep(2000);
    // Re-apply class after reload (in case React resets it during hydration)
    await cdp.evalJs("document.documentElement.classList.add('dark')");

    console.log(`── Mobile DARK MODE click-through (${MOBILE_WIDTH}x${MOBILE_HEIGHT}) ────────────\n`);
    ...
    console.log('');
    console.log('══════════════════════════════════════════════════════');
    console.log(`viewport: ${MOBILE_WIDTH}x${MOBILE_HEIGHT} (iPhone X) · dark mode`);
